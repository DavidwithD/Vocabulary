...
const lemmaCacheFr = new Map<string, string>();
 ...
 */
const FR_IRREGULAR = new Map<string, string>([
  // être
  ['suis', 'être'],
  ['es', 'être'],
  ['est', 'être'],
 ...
 */
export function lemmatizeFr(word: string): string {
  if (!word) return '';
  ...
  if (/^.{3,}(erai|eras|eront|erais|erait|erions|eriez|eraient)$/.test(lower)) {
    result = lower.replace(
      /(erai|eras|eront|erais|erait|erions|eriez|eraient)$/,
      'er'
    );
